...
const StoreFrontProductCard = ({
  ...
  if (viewMode === "list") {
    return (
      <motion.div
        ...
        className="group flex cursor-pointer overflow-hidden rounded-2xl border bg-card transition-all hover:shadow-lg"
      ...
      >
          ...
          <button
            ...
            className={`absolute right-1.5 top-1.5 flex h-8 w-8 items-center justify-center rounded-full shadow-md transition-all duration-200 hover:scale-110 ${
              isWishlisted
                ...
                : "bg-white/90 text-muted-foreground backdrop-blur-sm hover:bg-white"
            }`}
          >
        ...
        <div className="flex flex-1 flex-col justify-between p-3.5 sm:p-4">
          <div className="space-y-1">
            ...
            <h3 className="line-clamp-1 text-sm font-semibold text-foreground sm:text-base">{p.name}</h3>
            {p.description && (
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <Button
              className="ml-auto gap-1.5 rounded-xl text-xs text-white transition-all duration-150 hover:brightness-110 active:scale-95"
              size="sm"
  ...
  // Grid view — Demo style (rounded, square, hover CTA in primary color)
  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: 20 }}
    ...
    >
      <div className="relative aspect-square overflow-hidden rounded-2xl bg-muted">
        ...
        <button
          onClick={(e) => onToggleWishlist(p, e)}
          className={`absolute right-2 top-2 flex h-9 w-9 items-center justify-center rounded-full shadow-md backdrop-blur-sm transition-all duration-200 hover:scale-110 sm:right-2.5 sm:top-2.5 ${
            isWishlisted ? "bg-red-500 text-white hover:bg-red-500" : "bg-white/90 text-muted-foreground hover:bg-white"
          }`}
        >
